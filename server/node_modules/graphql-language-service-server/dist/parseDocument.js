...
function parseDocument(text, uri, fileExtensions = exports.DEFAULT_SUPPORTED_EXTENSIONS, graphQLFileExtensions = exports.DEFAULT_SUPPORTED_GRAPHQL_EXTENSIONS, logger = new Logger_1.Logger()) {
    const ext = (0, path_1.extname)(uri);
    ...
    if (graphQLFileExtensions.some(e => e === ext)) {
        ...
        const range = new graphql_language_service_1.Range(new graphql_language_service_1.Position(0, 0), new graphql_language_service_1.Position(lines.length - 1, lines[lines.length - 1].length - 1));
        return [{ query, range }];
    }
